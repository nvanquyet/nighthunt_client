    ...
    public class InventorySystem : NetworkBehaviour, IInventorySystem, IDisposable
    {
        ...
        private void ValidateReferences()
        {
            ...
            if (_gameplayConfig == null)
                Debug.LogWarning("[InventorySystem] GameplayConfig is null!");

            if (_inventoryConfig == null)
                Debug.LogWarning("[InventorySystem] InventoryConfig is null!");
            ...
            ItemInstanceData newValue, bool asServer)
        {
            if (asServer)
                return;
            // Host: server already updated all caches directly in Add/Remove/UpdateItemData.
            // Suppress the client-host SyncList callback to prevent duplicate cache entries
