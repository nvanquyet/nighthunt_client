...
namespace NightHunt.GameplaySystems.QuickSlot
{
    ...
    public class ProjectileNetworked : NetworkBehaviour
    {
        ...
        private void Awake()
        {
            _rb = ComponentResolver.Find<Rigidbody>(this)
                .OnSelf()
                .InChildren()
                .OrLogWarning("[Auto] Rigidbody not found")
                .Resolve();
            _projectileBase = ComponentResolver.Find<ProjectileBase>(this)
                .OnSelf()
                .InChildren()
                .OrLogWarning("[Auto] ProjectileBase not found")
                .Resolve();
        }
        ...
        public void Initialize(ThrowableDefinition def)
        {
            ...
            if (!def.CanBounce)
            {
                var col = ComponentResolver.Find<Collider>(this)
                    .OnSelf()
                    .InChildren()
                    .OrLogWarning("[Auto] Collider not found")
                    .Resolve();
                if (col != null)
                {
        ...
        private void DealAoeDamage()
        {
            ...
            {
                var damageable = ComponentResolver.Find<IDamageable>(hit)
                    .OnSelf()
                    .InChildren()
                    .OrLogWarning("[Auto] IDamageable not found")
                    .Resolve();
                if (damageable == null)
                    continue;
                ...
                // Physics impulse
                var rb = ComponentResolver.Find<Rigidbody>(hit)
                    .OnSelf()
                    .InChildren()
                    .OrLogWarning("[Auto] Rigidbody not found")
                    .Resolve();
