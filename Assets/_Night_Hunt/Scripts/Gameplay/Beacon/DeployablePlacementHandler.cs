...
namespace NightHunt.Gameplay.Beacon
{
    ...
    public sealed class DeployablePlacementHandler : NetworkBehaviour, IDeployableHandler
    {
        ...
        private bool TryGetPlacementPoint(out Vector3 point, out Quaternion rotation, out bool valid)
        {
            ...
            if (IsMobile)
            {
                Vector3 aimPoint = ThrowableAimController.AimWorldTarget;
                if (aimPoint.sqrMagnitude <= 0.0001f)
                {
                    aimPoint = _networkPlayer != null 
                        ? _networkPlayer.transform.position + _networkPlayer.transform.forward * 3f 
                ...
                valid = false;
                _lastPlacementRejectReason = $"mobileSurfaceMiss aimPoint={aimPoint:F2} rayOrigin={rayOrigin:F2} surfaceMask={surfaceMask.value}";
                return true;
            }
