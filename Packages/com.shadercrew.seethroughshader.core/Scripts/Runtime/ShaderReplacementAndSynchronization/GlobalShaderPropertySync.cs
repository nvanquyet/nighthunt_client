...
{
    [AddComponentMenu(Strings.COMPONENTMENU_GLOBAL_SHADER_PROPERTY_SYNC)]
    public class GlobalShaderPropertySync: ReplacementAndSyncBaseAbstract
    {
        ...
        private void updateShaderKeywords(bool force = false)
        {
            if (referenceMaterial != null)
            {
                if (transformsWithSTS != null && transformsWithSTS.Length > 0 && UnityToSTSShaderMapping != null)
                {
                    ...
                    GeneralUtils.updateSeeThroughShaderMaterialKeywordsAndCulling(transformsWithSTS, UnityToSTSShaderMapping.Values.ToList(), referenceMaterial, force);
