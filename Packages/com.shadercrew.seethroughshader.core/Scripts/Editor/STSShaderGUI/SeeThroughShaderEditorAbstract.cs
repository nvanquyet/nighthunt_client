...
#if USING_HDRP
            // Unity 6 fix: When both HDRP and URP packages are installed, USING_HDRP is defined
            // but the material may be a URP shader. material.HasProperty("_CullMode") can return
            // true even for URP materials (due to HDRP being present), but _CullMode won't be in
            // the props array. Use propertyIsMandatory=false and fall back to _Cull for URP shaders.
            if (material.HasProperty("_CullMode"))
            {
                seeThroughShaderGUI.cull = FindProperty("_CullMode", props, false);
                if (seeThroughShaderGUI.cull == null && material.HasProperty("_Cull"))
                {
                    seeThroughShaderGUI.cull = FindProperty("_Cull", props, false);
                }
            }
