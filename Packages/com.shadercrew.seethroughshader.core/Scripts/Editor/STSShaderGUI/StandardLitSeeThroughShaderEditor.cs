...
namespace ShaderCrew.SeeThroughShader
{
    public class StandardLitSeeThroughShaderEditor : SeeThroughShaderEditorAbstract
    {
        StandardLitGUI standardLitGUI;
                    ...
                    {
                        // Unity 6 fix: Only run HDRP validation for actual HDRP materials.
                        // When both HDRP and URP packages are installed USING_HDRP is always
                        // defined, but ValidateMaterial may be called on a URP material.
                        if (GeneralUtils.getCurrentRenderPipeline() == RenderPipeline.HDRP)
                            LitAPI.ValidateMaterial(material);
                    }
#else
